        ...
        <nav className="flex-1 overflow-y-auto py-2 space-y-0.5 px-1.5">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs font-mono transition-all border border-transparent ${collapsed ? "justify-center" : ""}`}
              activeProps={{
                className: "bg-cyan-500/10 text-cyan-400 border-cyan-500/20"
              }}
              inactiveProps={{
                className: "text-gray-500 hover:text-gray-300 hover:bg-white/3"
              }}
