...
export default function AdminResumePage() {
  ...
  useEffect(() => {
    if (data) {
      // Exclude Mongo fields for clean editing
  ...
  const updateMutation = useMutation({
    mutationFn: async (payload: any) => api.put("/resume", payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-resume-data"] });
      queryClient.invalidateQueries({ queryKey: ["resume"] });
      triggerSuccess("Resume data successfully updated.");
    },
    onError: (err: any) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!isValidJson) {
      setError("Please fix compilation errors in the JSON configuration.");
      return;
    }
    ...
    try {
      ...
      setError(`Failed to parse config: ${e.message}`);
    }
  };
  ...
  if (isLoading) {
    return <div className="text-center py-20 text-xs text-cyan-400 font-mono">// COMPILING_RESUME_REPOSITORIES...</div>;
  }

  return (
    <div className="max-w-4xl mx-auto space-y-6 font-mono text-sm">
      <div className="border-b border-white/5 pb-4">
        <h1 className="text-xl font-bold text-cyan-400">// EDIT_RESUME_MANIFEST</h1>
        <p className="text-xs text-gray-400 mt-1">Configure structured experiences, education, and credentials arrays</p>
      </div>
